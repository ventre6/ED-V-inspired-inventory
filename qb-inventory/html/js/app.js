...
$(document).on("mouseenter", ".item-slot", function(e) {
    ...
    if ($(this).data("item") != null) {
        ...
        $(".ply-iteminfo-container").fadeIn(1);
...
$(document).on('click', '#craft-btn', function(e){
    $.post("https://qb-inventory/OpenCraft", JSON.stringify({}));
});
...
$(document).mousemove(function(event) {
    ...
    let offsetX = 45; // İmlecin konumu ile element arasındaki yatay mesafe
    let offsetY = 20; // İmlecin konumu ile element arasındaki dikey mesafe

    // İmlecin X ve Y koordinatları
    let mouseX = event.pageX;
    let mouseY = event.pageY;

    // Elementin konumunu hesapla
    let elementX = mouseX + offsetX;
    let elementY = mouseY - offsetY;

    // Elementin sağa ve aşağı taşma durumlarını kontrol et
    if (elementX < 0) {
        elementX = 0;
    } else if (elementX + element.width() > $(window).width()) {
        elementX = $(window).width() - element.width();
    }

    if (elementY < 0) {
        elementY = 0;
    } else if (elementY + element.height() > $(window).height()) {
        elementY = $(window).height() - element.height();
    }

    // Elementin pozisyonunu güncelle
    ...
    if (itemData != null && itemData.info != "") {
        if (itemData.name == "id_card") {
            ...
            if (itemData.info.attachments != null) {
                ...
                $(".item-info-description").html(
                    "<p><strong>Seri Numarası: </strong><span>" +
                    itemData.info.serie +
                    "</span></p><p><strong>Mermi: </strong><span>" +
                    itemData.info.ammo +
                    "</span></p><p><strong>Eklentiler: </strong><span>" +
                    attachmentString +
                    "</span></p>"
                );
            } else {
                ...
                if (itemData.price != null) {
                    ...
                    $(".ui-draggable-dragging")
                        .find(".item-slot-amount p")
                         //  .html(" " + itemData.amount + " $" + itemData.price); // para ile akalakı
            ...
            if (fromData.amount == $toAmount) {
                ...
                if (newData.name.split("_")[0] == "weapon") {
                    if (!Inventory.IsWeaponBlocked(newData.name)) {
                        ...
                        var QualityColor = "rgb(113 255 122)";
...
(() => {
    ...
    Inventory.ToggleHotbar = function(data) {
        if (data.open) {
            ...
            var elem =
                '<div class="z-hotbar-item-slot" data-zhotbarslot="5"> <div class="z-hotbar-item-slot-key"><p>5<div class="z-hotbar-item-slot-img"></div><div class="z-hotbar-item-slot-label"><p>&nbsp;</p></div></div>';
            $(".z-hotbar-inventory").append(elem);
