...
RegisterNUICallback("OpenCraft", function(data, cb)
    if IsPedInAnyVehicle(PlayerPedId(), false) then
        QBCore.Functions.Notify("Araçtayken üret menüsünü kullanamazsın!", "error")
        return
        else
...
RegisterNetEvent('qb-weapons:client:SetWeaponAmmoManual', function(weapon, ammo)
    ...
    if weapon ~= "current" then
        ...
        QBCore.Functions.Notify("Mermi verdim", 'success')
    else
        ...
        if weapon ~= nil then
            ...
            QBCore.Functions.Notify("Elinde silah yok", "error")
        end
    end
end)
