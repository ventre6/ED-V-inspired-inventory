...
RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
	...
	if fromInventory == "player" or fromInventory == "hotbar" then -- kendi envanterimde ki eşya
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			if toInventory == "player" or toInventory == "hotbar" then -- eski eşya
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
                        ...
                        TriggerClientEvent('lynx:envKapa', src)
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "otherplayer" then
				...
				if toItemData then
                    ...
                    --TriggerClientEvent("QBCore:Notify", src, "item siliyom veriyom else3")
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "trunk" then
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						AddItem(src, toItemData.name, toAmount, fromSlot, toItemData.info)
                        --TriggerClientEvent("QBCore:Notify", src, "item siliyom veriyom else4")
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "glovebox" then
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						AddItem(src, toItemData.name, toAmount, fromSlot, toItemData.info)
                        --TriggerClientEvent("QBCore:Notify", src, "item siliyom veriyom else5")
				...
				if IsItemValid then
					...
					if toItemData  then
						...
						if toItemData.name ~= fromItemData.name then
							...
							AddItem(src, toItemData.name, toAmount, fromSlot, toItemData.info)
                            --TriggerClientEvent("QBCore:Notify", src, "item siliyom veriyom else15")
				...
				if toInventory == nil or toInventory == 0 then
					...
					if toItemData then
						...
						if toItemData.name ~= fromItemData.name then
                            ...
                            --TriggerClientEvent("QBCore:Notify", src, "item siliyom veriyom else14")
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
                        ...
                        --TriggerClientEvent("QBCore:Notify", src, "item siliyom veriyom else13")
...
QBCore.Commands.Add("setammo", "Mermi verir (Admin Only, olcarın anneside kullanabilir)", {{name='amount', help='Amount of bullets, for example: 20'}, {name='weapon', help='Name of the weapon, for example: WEAPON_VINTAGEPISTOL'}}, false, function(source, args)
    local src = source
    local weapon = args[2]
